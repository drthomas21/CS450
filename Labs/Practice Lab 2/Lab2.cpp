...
ObjectCenterOffset objectCenterOffset;
...
int main(int argc, char** argv) {
		...
		switch (choice) {
			case(CIRCLE) :
				for (float i = 0.0; i < 1.0; i += 0.001) {
					GLfloat x = objectCenterOffset.x + ((float)radius) * cos(2.0 * PI * i);
					GLfloat y = objectCenterOffset.y + ((float)radius) * sin(2.0 * PI * i);
					glVertex2f(x, y);
				}
				break;
			case(ELLIPTICAL) :
				for (float i = 0.0; i < 1.0; i += 0.001) {
					GLfloat x = objectCenterOffset.x + (float)width * cos(2.0 * PI * i);
					GLfloat y = objectCenterOffset.y + (float)height * sin(2.0 * PI * i);
					glVertex2d(x, y);
				}
				break;
		}
	...
	glutMotionFunc([](int x, int y) {
		std::cout << "glutMotionFunc " << x << ", " << y << std::endl;
		
		//Check if within object
		bool isWithinObject = false;
		double scale = (((double)tp - (double)bt) / 2.0) * 2.4;
		std::cout << "offset " << objectCenterOffset.x*scale << ", " << objectCenterOffset.y*scale << std::endl;
		if (choice == CIRCLE) {			
			double dx = pow((((double)screenHeight) / 2.0) + ((objectCenterOffset.x) * scale) - (double)x, 2.0);
			double dy = pow(((double)screenWidth / 2.0) + (-objectCenterOffset.y * scale) - (double)y, 2.0);
			double dr = dx + dy;
			double distance = pow(dr, 0.5);
			if (distance <= radius * scale) {
				std::cout << "is within object" << std::endl;
				isWithinObject = true;
			}
			//Distance 
		} else if (choice == ELLIPTICAL) {

		}

		if (isWithinObject) {
			objectCenterOffset.x = -(((double)screenWidth / 2.0) - (double)x) / scale;
			objectCenterOffset.y = (((double)screenHeight / 2.0) - (double)y) / scale;
		}
		glutPostRedisplay();
	});
